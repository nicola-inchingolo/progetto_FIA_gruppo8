...
class TestHoldoutEvaluator(unittest.TestCase):
    ...
    def test_init_valid(self):
        ev = holdout_evaluator(self.df, np.array([1]), 0.5, 2)
    ...
    def test_split_dataset_missing_columns(self):
        ...
        ev = holdout_evaluator(df, np.array([1]), 0.5, 2)
    ...
    def test_holdout_evaluation(self):
        ...
        evaluation = holdout_evaluator(self.df,features, train_percentage, 2)

        x_train, x_test, y_train, y_test = holdout_evaluator.split_dataset_with_strategy()
        ...
        model = Knn_Algorithm.KNNClassifier(k=5, p=evaluation.distance_strategy)
